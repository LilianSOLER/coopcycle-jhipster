...
type RestOf<T extends IDelivery | NewDelivery> = Omit<T, 'deliveryTime' | 'pickupTime'> & {
...
export class DeliveryService {
  ...
  protected convertDateFromClient<T extends IDelivery | NewDelivery | PartialUpdateDelivery>(delivery: T): RestOf<T> {
    return {
      ...delivery,
      deliveryTime: delivery.deliveryTime?.toJSON() ?? null,
      pickupTime: delivery.pickupTime?.toJSON() ?? null,
    };
  }
  ...
  protected convertDateFromServer(restDelivery: RestDelivery): IDelivery {
    return {
      ...restDelivery,
      deliveryTime: restDelivery.deliveryTime ? dayjs(restDelivery.deliveryTime) : undefined,
      pickupTime: restDelivery.pickupTime ? dayjs(restDelivery.pickupTime) : undefined,
    };
  }
